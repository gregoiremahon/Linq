...
int windowPositionsX[5] = { 100, 300, 500, 700, 900 };
int windowPositionsY[5] = { 100, 100, 100, 100, 100 };
...
void manageRedraw()
{
    ...
    case 1:
    {
        ...
        for (int i = 0; i < 5; i++)
        {
            SDL_Rect rect = { windowPositionsX[i], windowPositionsY[i], 200, 50 };
            SDL_RenderCopy(renderer, texture_connectbutton, NULL, &rect);
            if (i < cptWord)
                myRenderText(gNames[i], windowPositionsX[i] + 5, windowPositionsY[i] + 5);
        }
    }
    break;
    default:
